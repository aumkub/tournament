...
export async function action({ request, params, context }: Route.ActionArgs) {
	...
	// Check attendee-specific limit
	if (tournament.attendee_limit) {
		const count = await env.DB.prepare(
			"SELECT COUNT(*) as cnt FROM registrations WHERE tournament_id = ? AND type = 'attendee'",
		)
			.bind(tournament.id as string)
			.first();
		if (count && (count.cnt as number) >= (tournament.attendee_limit as number)) {
			return Response.json(
				{ error: "ขออภัย ที่นั่งผู้เข้าร่วมงานเต็มแล้ว" },
				{ status: 400 },
			);
		}
	}
	...
	await env.DB.prepare(
     ...
     VALUES (?, ?, 'attendee', ?, ?, ?, 0, ?)`,
		...
		.bind(id, tournament.id as string, body.email, JSON.stringify(body.data), token, now)
		.run();
